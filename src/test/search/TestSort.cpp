...
Query* sort_queryF;
Sort* _sort;
SimpleAnalyzer<TCHAR> sort_analyser;
...
Searcher* sort_getIndex (bool even, bool odd){
	...
	for (int i=0; i<11; ++i) {
		if (((i%2)==0 && even) || ((i%2)==1 && odd)) {
			Document doc;
			doc.add (*_CLNEW Field ( _T("tracer"),   data1[i][0], Field::STORE_YES));
			doc.add (*_CLNEW Field ( _T("contents"), data1[i][1], Field::INDEX_TOKENIZED));
			if (data1[i][2] != NULL)
                doc.add (*_CLNEW Field (_T("int"),   data1[i][2], Field::INDEX_UNTOKENIZED));
			if (data1[i][3] != NULL)
                doc.add (*_CLNEW Field (_T("float"),    data1[i][3], Field::INDEX_UNTOKENIZED));
			if (data1[i][4] != NULL)
                doc.add (*_CLNEW Field (_T("string"),   data1[i][4], Field::INDEX_UNTOKENIZED));
			if (data1[i][5] != NULL)
                doc.add (*_CLNEW Field (_T("custom"),   data1[i][5], Field::INDEX_UNTOKENIZED));

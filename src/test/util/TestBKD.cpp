...
void testBug1Read(CuTest *tc) {
    ...
    Directory *dir(FSDirectory::getDirectory("TestBKDTree"));
    {
        ...
        auto meta_in_ =  std::unique_ptr<IndexInput>(dir->openInput("bkd3_meta"));
        auto index_in_ =  std::unique_ptr<IndexInput>(dir->openInput("bkd3_index"));

        shared_ptr<bkd::bkd_reader> r = make_shared<bkd::bkd_reader>(in_.release());
        // Simple 1D range query:
        int value = 0;
...
void testLowCardinalInts1DRead2(CuTest *tc) {
    ...
    Directory *dir(FSDirectory::getDirectory("TestBKDTree"));
    {
        ...
        auto meta_in_ =  std::unique_ptr<IndexInput>(dir->openInput("bkd2_meta"));
        auto index_in_ =  std::unique_ptr<IndexInput>(dir->openInput("bkd2_index"));
...
void testBasicsInts1DRead(CuTest *tc) {
    uint64_t str = Misc::currentTimeMillis();
    const int N = 1024 * 1024;
    auto dir = std::unique_ptr<Directory>(FSDirectory::getDirectory("TestBKDTree"));
    std::unique_ptr<IndexInput> in_ = std::unique_ptr<IndexInput>(dir->openInput("bkd"));;

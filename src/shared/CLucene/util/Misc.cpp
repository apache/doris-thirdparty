...
template <typename T>
size_t Misc::swhashCode(const T* str, size_t len){
    // Compute the hash code using a local variable to be reentrant.
    size_t hashCode = 0;
    for (size_t i = 0; i<len; i++)
        hashCode = hashCode * 31 + *str++;
...
template size_t Misc::swhashCode(const char* str, size_t len);
template size_t Misc::swhashCode(const TCHAR* str, size_t len);
...
wchar_t *Misc::_charToWide(const char *s, size_t len)
{
  wchar_t *msg = _CL_NEWARRAY(wchar_t, len + 1);
...
bool Misc::listDirs(const char* directory, std::vector<std::string>& files, bool fullPath){
    //clear old files
...
//bool Misc::zstd_compress(const uint8_t* source, size_t sourcelen, std::ostream& dest, std::string& err, int CHUNK, int level)
//{
//  size_t const cBuffSize = ZSTD_compressBound(sourcelen);
//  void* cBuff = malloc(cBuffSize);
//  if (!cBuff)
//  {
//    zerr(Z_MEM_ERROR, err);
//    return false;
//  }
//
//  if ( level == -1 ) level = 3;
//
//  size_t const cSize = ZSTD_compress(cBuff, cBuffSize, source, sourcelen, level);
...
//  }
//
//  dest.write((char *)cBuff, cSize);
//  return true;
//}
//
//bool Misc::zstd_decompress(const uint8_t* source, size_t sourcelen, std::ostream& dest, std::string& err, int CHUNK)
//{
//  unsigned long long const rSize = ZSTD_getFrameContentSize(source, sourcelen);
...
//  }
//
//  size_t const dSize = ZSTD_decompress(rBuff, rSize, source, sourcelen);

...
int ThesaurusDict::load(const char* filename) {
  ...
  _csr_thesaurusdict_fileheader* head_ = reinterpret_cast<_csr_thesaurusdict_fileheader*>(ptr);
...
int ThesaurusDict::import(const char* filename, const char* target_file) {
  ...
  while (std::getline(*is, line)) {
    if (n % 2) {
      ...
      // the value row
      ThesaurusRecord* tr =
          new ThesaurusRecord;  // FIXME: should free, but who care
      tr->key = key;
      memset(tr->value, 0, sizeof(tr->value));
      memcpy(tr->value, &line.c_str()[1], line.length() - 1);
      tr->length = static_cast<u2>(line.length());
      u1* ptr = tr->value;
      ...
      items.push_back(tr);
      string_bufsize += static_cast<int>(line.length());  // append addtional \0's space
      continue;
    }
  ...
  u1* total_buf = static_cast<u1*>(malloc(string_bufsize));
  memset(static_cast<void*>(total_buf), 0, string_bufsize);
  u1* total_buf_ptr = total_buf;
  // read complete, try make dict
  std::sort(items.begin(), items.end(), Cmp);
  {
    std::vector<Darts::DoubleArray::key_type*> key_tmp;
    ...
    for (i = 0; i < items.size(); i++) {
      ...
      char* ptr = &rec->key[0];
      key_tmp.push_back(ptr);
      memcpy(total_buf_ptr, rec->value, rec->length);
      value.push_back(static_cast<int>(total_buf_ptr -
                            total_buf));  // value is the string_pool's offset
    ...
    //int nRet =
    m_da.build(key_tmp.size(), &key_tmp[0], 0, &value[0]);
  ...
  // free it
  free(static_cast<void*>(total_buf));
  return 0;
}
...
const char* ThesaurusDict::find(const char* key, u2 key_len, int* count) {
  ...
  *count = 0; //no use
  ...
  if (rs.pos && rs.value >= 0) {
    ...
    return reinterpret_cast<const char*>(&m_stringpool[offset]);
  }
  return NULL;
}

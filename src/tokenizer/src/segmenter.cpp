...
1, det is too small
*/
int Segmenter::toLowerCpy(__attribute__((unused)) const u1* src, __attribute__((unused)) u1* det, __attribute__((unused)) u2 det_size) {
    return 0;
}
...
void Segmenter::setBuffer(u1* buf, u4 length) {
  ...
  if (!m_tagger) m_tagger = ChineseCharTagger::Get();
  m_thunk.reset();
  return;
}
...
u1 Segmenter::isSentenceEnd() {
  u2 aLen = 0;
  //u1* tok =
  m_thunk.peekToken(aLen);
  return (aLen == 0);
}

int Segmenter::isKeyWord(u1* buf, __attribute__((unused)) u4 length) {
  if (!m_kwdict) return 0;
  int id = m_kwdict->exactMatch(reinterpret_cast<const char*>(buf));
  return (id > 0);
}

int Segmenter::getWordWeight(u1* buf, u4 length) {
  if (!m_weightdict) return length;  // use token length as weight
  int count = m_weightdict->exactMatch(reinterpret_cast<const char*>(buf));
...
int dry_strchr(const u1* pattern, u2 iCode) {
  ...
  while (*ptr) {
    if (*ptr == iCode) return ptr - pattern;
    ptr++;
  }
  if (!*ptr) return -1;
  return -1;
...
const u1* Segmenter::peekToken(u2& aLen, u2& aSymLen, u2 n) {
  ...
  // reset unigram when
  // get token
  if (n == 1) return peekKwToken(aLen, aSymLen);
  u2 len;
  ...
  if (aLen) {
    ...
    // check sym
    int sym_key_len = 64;
    const char* sym = m_symdict->maxMatch(reinterpret_cast<const char*>(tok), sym_key_len);
    if (sym) {
      aSymLen = static_cast<u2>(strlen(sym));
      aLen = sym_key_len;
      /*
      ...
      */
      return reinterpret_cast<const u1*>(sym);
    }  // end sym'
    aSymLen = aLen;
    return tok;
  }
  ...
  while (*ptr && (ptr < m_buffer_end) && i < CHUNK_BUFFER_SIZE) {
    ...
    // check tagger
    int num = m_unidict->findHits(reinterpret_cast<const char*>(ptr), &rs[1], 1024 - 1,
                                  MAX_TOKEN_LENGTH);
    if (num) {
      if (rs[1].length == len)
    ...
    if (m_kwdict)
      num = m_kwdict->findHits(reinterpret_cast<const char*>(ptr), &rs[1], 1024 - 1,
                               MAX_TOKEN_LENGTH);
    else
      num = 0;
  ...
  if (tag == 'm' || tag == 'e') {
    ...
    while (*ptr) {
      ...
      */
      if ((tag == 'm') && m_config && m_config->seperate_number_ascii) break;
      // if(tag != prev_tag)
      /*
              here  needs to check more rules.
              - if merge_number_and_ascii=true, and tag = m, prev_tag = e, or
         reserve, do NOT break;
      ...
      */
      if (tag != prev_tag) {
        ...
        if (m_config && !bContinue)
          bContinue =
              (-1 != dry_strchr(m_config->number_and_ascii_joint, iCoden));
        if (m_config && !bContinue)
          bContinue =
              (-1 != dry_strchr(m_config->number_and_ascii_joint, iCodep));

        if (m_config && m_config->seperate_number_ascii) bContinue = 0;
  ...
  if (m_thunk.length()) return peekToken(aLen, aSymLen);
  /*
...
void Segmenter::popKwToken(__attribute__((unused)) u2 len) { return; }

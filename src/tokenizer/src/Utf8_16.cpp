...
int csrUTF8Encode(u1* pBuf, int iCode) {
  if (iCode < 0x80) {
    pBuf[0] = static_cast<u1>(iCode & 0x7F);
    return 1;

  } else if (iCode < 0x800) {
    pBuf[0] = static_cast<u1>(((iCode >> 6) & 0x1F) | 0xC0);
    pBuf[1] = static_cast<u1>((iCode & 0x3F) | 0x80);
    return 2;

  } else {
    pBuf[0] = static_cast<u1>(((iCode >> 12) & 0x0F) | 0xE0);
    pBuf[1] = static_cast<u1>(((iCode >> 6) & 0x3F) | 0x80);
    pBuf[2] = static_cast<u1>((iCode & 0x3F) | 0x80);
    return 3;
  }
}
...
int csrUTF8Decode(const u1* pBuf, u2& length) {
  ...
  iBytes--;
  do {
    if (!(*pBuf)) return 0;  // unexpected eof

    if (((*pBuf) & 0xC0) != 0x80) return -1;  // invalid code
...
int Utf8_16_Read::determineEncoding() {
  ...
  if (m_nLen > 1) {
    if (m_pBuf[0] == k_Boms[eUtf16BigEndian][0] &&
        m_pBuf[1] == k_Boms[eUtf16BigEndian][1]) {
      m_eEncoding = eUtf16BigEndian;
      nRet = 2;
    } else if (m_pBuf[0] == k_Boms[eUtf16LittleEndian][0] &&
               m_pBuf[1] == k_Boms[eUtf16LittleEndian][1]) {
      m_eEncoding = eUtf16LittleEndian;
      nRet = 2;
    } else if (m_nLen > 2 && m_pBuf[0] == k_Boms[eUtf8][0] &&
               m_pBuf[1] == k_Boms[eUtf8][1] && m_pBuf[2] == k_Boms[eUtf8][2]) {
      m_eEncoding = eUtf8;
      nRet = 3;
    }
  }
...
size_t Utf8_16_Write::fwrite(const void* p, size_t _size) {
  ...
  if (m_eEncoding == eUtf8) {
    if (m_bFirstWrite) ::fwrite(k_Boms[m_eEncoding], 3, 1, m_pFile);
    m_bFirstWrite = false;
    return ::fwrite(p, _size, 1, m_pFile);
  }
  ...
  size_t ret =
      ::fwrite(m_pBuf, reinterpret_cast<const char*>(pCur) - reinterpret_cast<const char*>(m_pBuf), 1, m_pFile);

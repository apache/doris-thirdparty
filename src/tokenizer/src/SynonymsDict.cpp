...
int SynonymsDict::load(const char* filename) {
  ...
  if (!m_file) return -1;  // can not load dict.
  csr_offset_t tm_size = csr_mmap_size(m_file);
  u1* ptr = static_cast<u1*>(csr_mmap_map(m_file));
  u1* ptr_end = ptr + tm_size;
  _csr_synonymsdict_fileheader* head_ = reinterpret_cast<_csr_synonymsdict_fileheader*>(ptr);
  if (strncmp(head_->mg, head_mgc, 4) == 0) {
    if (head_->version == 1) {
      ptr += sizeof(_csr_synonymsdict_fileheader);
      if (ptr >= ptr_end) return -4;  // file broken.
      array_ = reinterpret_cast<_csr_sybarray_trie*>(ptr);
      ptr += sizeof(_csr_sybarray_trie) * head_->darts_size;
      if (ptr >= ptr_end) return -4;  // file broken.
      string_pool = reinterpret_cast<char*>(ptr);
...
int SynonymsDict::import(const char* filename) {
  ...
  while (std::getline(*is, line)) {
    ...
    if (lKey.size() == 0 || rKey.size() == 0) continue;
    // all key loaded.
    // check rKey
    ...
    if (it == lKeys.end()) {
      lKeys.insert(
          std::map<std::string, size_t>::value_type(lKey, lKey_offset));
    } else {
      // dup left Keys! skip.
    }
  }  // end while
...
int SynonymsDict::save(const char* filename) {
  ...
  for (std::map<std::string, size_t>::iterator it = lKeys.begin();
    ...
    // char* ptr = &(str.data())[0];
    //char* ptr = const_cast<char*>(&((*it).first)[0]));
    keys.push_back(const_cast<char*>(it->first.c_str()));
  }
  std::sort(keys.begin(), keys.end(), Cmp);
  m_da.clear();
  //int nRet =
  m_da.build(keys.size(), &keys[0], 0, 0);
  ...
  for (size_t i = 0; i < size_; i++) {
    array_[i].base = static_cast<i4>(const_cast<int*>(iArray)[i * 2]);
    array_[i].check = static_cast<u4>(const_cast<int*>(iArray)[i * 2 + 1]);
    array_[i].offset = 0;
  }
  // fill offset
  for (std::map<std::string, size_t>::iterator it = lKeys.begin();
...
int SynonymsDict::exactMatchID(const char* key) {
  ...
  if (static_cast<array_u_type_>(b) == array_[p].check && n < 0)
    set_result(result, -n - 1, static_cast<u1>(len), p);
  if (result.dict_id) return result.dict_id;
  return -1;
};

const char* SynonymsDict::maxMatch(const char* key, int& len) {
  if (!array_) return NULL;
  ...
  array_type_ b = array_[0].base;  // node_pos = 0;
  size_t num = 0;
  array_type_ n;
  array_u_type_ p;

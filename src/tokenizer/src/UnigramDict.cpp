...
/**
     ...
     */
int UnigramDict::findHits(const char* buf, result_pair_type* result,
                          size_t result_len, int keylen) {
  if (!m_da.array()) return 0;
...
int UnigramDict::isLoad() { return m_da.array() != NULL; }

int UnigramDict::exactMatch(const char* key, int* id) {
  Darts::DoubleArray::result_pair_type rs;
  m_da.exactMatchSearch(key, rs);
  if (id) *id = rs.pos;
  if (rs.pos) return rs.value;
  /// FIXME: this totaly a mixture. some single char's id > 0 if it in unigram
  /// input text, while other's id < 0 if not in ungram text.
  ...
  if (len < 4) {
    ...
    //v < 128 &&  // Oney: v always < 128
    if (len == 1 && id) *id = -1 * static_cast<int>(v);
    // get number of bytes
    int iBytes = 0;
    //int iBytesLength = 0;
    while (v & 0x80) {

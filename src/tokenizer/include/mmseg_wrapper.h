...
class ThreadHashLock : public HashLockSupplier {
 ...
 public:
  ...
  virtual void Lock(uint32_t hash) override { locks_[hash % locks_num_].lock(); }

  virtual void unlock(uint32_t hash) override { locks_[hash % locks_num_].unlock(); }
...
class MMSEGWrapper {
 ...
 public:
  ...
  void Init(__attribute__((unused)) const char *dict_path, size_t segmenter_num = 5000) {

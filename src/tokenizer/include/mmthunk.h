...
inline bool equal_approx(float const f1, float const f2)
{
    const double a = static_cast<double>(f1);
    const double b = static_cast<double>(f2);
 ...
 public:
  ...
  inline float get_free() {
    ...
    for (it = freqs.begin(); it < freqs.end(); it++) {
      freq = (static_cast<float>(*it)) + 1;
      score += log(freq) * 100;
    }
    return score;
  }
  inline float get_avl() {
    float avg = static_cast<float>(1.0) * total_length / tokens.size();
    return avg;
  }
  inline float get_avg() {
    ...
    return static_cast<float>(1.0) * total / (tokens.size() - 1);
  }
  inline void popup() {
    if (tokens.size()) {
...
// debug use->dump chunk
    ...
    for (size_t i = 0; i < m_chunks.size(); i++) {
      ...
      if (avl > avg_length) {
        avg_length = avl;
        k_ptr = remains;
        *k_ptr = static_cast<u4>(i);
        k_ptr++;
      } else if (equal_approx(avl,avg_length)) {
        *k_ptr = static_cast<u4>(i);
        k_ptr++;
      }
    }
    ...
    for (auto i = 0; i < k_ptr - remains; i++) {
      float avg = m_chunks[remains[i]].get_avg();
      if (avg < avg_length) {
        avg_length = avg;
        k_ptr_r3 = remains_r3;
        *k_ptr_r3 = static_cast<u4>(remains[i]);  //*k_ptr_r3 = (u4)i;
        k_ptr_r3++;
      } else if (equal_approx(avg,avg_length)) {
        *k_ptr_r3 = static_cast<u4>(i);;
        k_ptr_r3++;
      }
    }
    ...
    float max_score = 0.0;
    size_t idx = -1;
    for (auto i = 0; i < k_ptr_r3 - remains_r3; i++) {

...
DocumentsWriter::ThreadState::ThreadState(DocumentsWriter *__parent) : postingsFreeListTS(ValueArray<Posting *>(256)),
...
void DocumentsWriter::ThreadState::processDocument(Analyzer *analyzer) {
    ...
    if (_parent->ramBufferSize != IndexWriter::DISABLE_AUTO_FLUSH && _parent->numBytesUsed > 0.95 * _parent->ramBufferSize)
        _parent->balanceRAM();
}
...
DocumentsWriter::ThreadState::FieldData::FieldData(DocumentsWriter *__parent, ThreadState *__threadState, FieldInfo *fieldInfo) : docFields(ValueArray<Field *>(1)),
                                                                                                                                  _parent(__parent),
            ...
            for (;;) {
                ...
                if (maxFieldLength != IndexWriter::FIELD_TRUNC_POLICY__WARN) {
                    ...
                    _i64tot(IndexWriter::DEFAULT_MAX_FIELD_LENGTH,
                            defaultMaxAsChar, 10);
                    int32_t errMsgLen = _tcslen(errMsgBase) + _tcslen(fieldInfo->name) + _tcslen(defaultMaxAsChar);
...
void DocumentsWriter::ThreadState::FieldData::addPosition(Token *token) {
    ...
    // array
    const TCHAR *tokenText = token->termBuffer<TCHAR>();
    const int32_t tokenTextLen = token->termLength<TCHAR>();

...
SegmentReader::Norm::Norm(IndexInput *instrm, bool _useSingleNormStream, int32_t n, int64_t ns, SegmentReader *r, const char *seg) : number(n),
                                                                                                                                     normSeek(ns),
                                                                                                                                     _this(r),
                                                                                                                                     segment(seg),
...
SegmentReader *SegmentReader::get(SegmentInfo *si, bool doOpenStores) {
    // set doOpenStores false force
    return get(si->dir, si, NULL, false, false, BufferedIndexInput::BUFFER_SIZE, false);
}

SegmentReader *SegmentReader::get(SegmentInfo *si, int32_t readBufferSize, bool doOpenStores) {
    // set doOpenStores false force
    return get(si->dir, si, NULL, false, false, readBufferSize, false);
}
...
SegmentReader::SegmentReader() : DirectoryIndexReader(),
                                 _norms(false, true) {
}
...
bool SegmentReader::hasProx() {
    return _fieldInfos->hasProx();
}
...
SegmentReader *SegmentReader::reopenSegment(SegmentInfo *si) {
    SCOPED_LOCK_MUTEX(THIS_LOCK)
    bool deletionsUpToDate = (this->si->hasDeletions() == si->hasDeletions()) && (!si->hasDeletions() || this->si->getDelFileName().compare(si->getDelFileName()) == 0);

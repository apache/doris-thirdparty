...
void MultiLevelSkipListReader::SkipBuffer::readBytes(uint8_t* b, const int32_t len, int32_t offset) {
	memcpy(b+offset,data+pos,len*sizeof(uint8_t));
	pos += len;
}
...
void DefaultSkipListReader::init(const int64_t _skipPointer, const int64_t freqBasePointer,
                                 const int64_t proxBasePointer, const int32_t df,
                                 const bool hasProx, const bool storesPayloads) {

...
AbortException::AbortException(CLuceneError &_err, DocumentsWriter *docWriter) : err(_err) {
    docWriter->setAborting();
}

DocumentsWriter::DocumentsWriter(CL_NS(store)::Directory *directory, IndexWriter *writer) : bufferedDeleteTerms(_CLNEW TermNumMapType(true, true)),
...
void DocumentsWriter::writeSegment(std::vector<std::string> &flushedFiles) {
    ...
    if (infoStream != NULL) {
        ...
        (*infoStream) << string("  oldRAMSize=") << Misc::toString(numBytesUsed) << string(" newFlushedSize=") << Misc::toString(newSegmentSize) << string(" docs/MB=") << Misc::toString((float_t) (numDocsInRAM / (newSegmentSize / 1024.0 / 1024.0))) << string(" new/old=") << Misc::toString((float_t) (100.0 * newSegmentSize / numBytesUsed)) << string("%\n");
    }
...
const TermNumMapType &DocumentsWriter::getBufferedDeleteTerms() {
    SCOPED_LOCK_MUTEX(THIS_LOCK)
...
void DocumentsWriter::clearBufferedDeletes() {
    SCOPED_LOCK_MUTEX(THIS_LOCK)
    TermNumMapType::iterator term = bufferedDeleteTerms->begin();
...
void DocumentsWriter::addDeleteTerm(Term *term, int32_t docCount) {
    ...
    if (num == NULL) {
        ...
        numBytesUsed += (_tcslen(term->field()) + term->textLength()) * BYTES_PER_CHAR + 4 + 5 * OBJECT_HEADER_BYTES + 5 * OBJECT_POINTER_BYTES;
...
bool DocumentsWriter::hasProx() { return fieldInfos->hasProx(); }
...
void DocumentsWriter::ByteSliceReader::readBytes(uint8_t *b, int32_t len, int32_t offset) {
    while (len > 0) {
...
DocumentsWriter::ByteBlockPool::ByteBlockPool(bool _trackAllocations, DocumentsWriter *_parent) : BlockPool<uint8_t>(_parent, BYTE_BLOCK_SIZE, _trackAllocations) {
}
...
DocumentsWriter::CharBlockPool::CharBlockPool(DocumentsWriter *_parent) : BlockPool<TCHAR>(_parent, CHAR_BLOCK_SIZE, false) {
}

...
SegmentTermEnum::SegmentTermEnum(const SegmentTermEnum& clone) : fieldInfos(clone.fieldInfos) {
    //Func - Constructor
...
void SegmentTermEnum::seek(const int64_t pointer, const int32_t p, Term* t, TermInfo* ti) {
    ...
    if (_term == NULL || _LUCENE_ATOMIC_INT_GET(_term->__cl_refcount) > 1) {
        ...
        _term = _CLNEW
                Term; //cannot use reference, because TermInfosReader uses non ref-counted array
    }
    _term->set(t, t->text());

...
void TermInfosReader::setIndexDivisor(const int32_t _indexDivisor) {
    if (indexDivisor < 1) _CLTHROWA(CL_ERR_IllegalArgument, "indexDivisor must be > 0");

    if (indexTerms != NULL) _CLTHROWA(CL_ERR_IllegalArgument, "index terms are already loaded");
...
void TermInfosReader::ensureIndexIsRead() {
    ...
    try {
        ...
        CND_CONDITION(
                indexTerms != NULL,
                "No memory could be allocated for indexTerms"); //Check if is indexTerms is a valid array
        ...
        CND_CONDITION(
                indexInfos != NULL,
                "No memory could be allocated for indexInfos"); //Check if is indexInfos is a valid array
        ...
        CND_CONDITION(
                indexPointers != NULL,
                "No memory could be allocated for indexPointers"); //Check if is indexPointers is a valid array
    ...
    _CLFINALLY(indexEnum->close();
               ...
               _CLDELETE(indexEnum->input); _CLDELETE(indexEnum); indexEnum = NULL;);
}
...
void TermInfosReader::seekEnum(const int32_t indexOffset) {
    ...
    SegmentTermEnum* enumerator = getEnum();
    enumerator->seek(indexPointers[indexOffset], (indexOffset * totalIndexInterval) - 1,

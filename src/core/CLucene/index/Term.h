...
*/
template<typename T>
class CLUCENE_EXPORT STerm:LUCENE_REFBASE {
private:
...
public:
    STerm(const STerm* fieldTerm, const T* txt);
    STerm();
    STerm(const TCHAR* fld, const T* txt, bool internField);
    STerm(const TCHAR* fld, const T* txt);
    ~STerm();

    const TCHAR* field() const; ///<returns reference

    const T* text() const; ///<returns reference

    ///Resets the field and text of a Term.
    inline void set(const TCHAR* fld, const T* txt){
        set(fld,txt,true);
    }
    void set(const STerm* term, const T* txt);

    void set(const TCHAR* fld, const T* txt, const bool internField);

    int32_t compareTo(const STerm* other) const;

    int32_t hashedCompareTo(STerm* other);

    bool equals(const STerm* other) const;

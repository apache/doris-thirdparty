...
/** Returns an enumeration of all terms starting at a given term. If
  ...
  * @memory Caller must clean up
	*/
	TermPositions* termPositions(Term* term, bool load_stats = false, const void* io_ctx = nullptr);
  ...
  * @memory Caller must clean up
	*/
	TermDocs* termDocs(Term* term, bool load_stats = false, const void* io_ctx = nullptr);

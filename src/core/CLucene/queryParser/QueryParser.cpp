...
Query* QueryParser::getFieldQuery(const TCHAR* _field, TCHAR* queryText) {
  ...
  if (v.size() == 0)
    ...
    Term* tm = _CLNEW Term(_field, v.at(0)->termBuffer<TCHAR>());
    ...
    if (severalTokensAtSamePosition) {
      if (positionCount == 1) {
        ...
        for(size_t i=0; i<v.size(); i++ ){
          Term* tm = _CLNEW Term(_field, v.at(i)->termBuffer<TCHAR>());
        ...
        return q;
            ...
            multiTerms.toArray(termsArray.values);
			    ...
			    multiTerms.push_back(_CLNEW Term(field, t->termBuffer<TCHAR>()));
		    }
        ValueArray<Term*> termsArray(multiTerms.size());
        multiTerms.toArray(termsArray.values);
		    if (enablePositionIncrements) {
      ...
      for (size_t i = 0; i < v.size(); i++) {
        t = v.at(i);
        Term* tm = _CLNEW Term(_field, t->termBuffer<TCHAR>());
        if (enablePositionIncrements) {
          position += t->getPositionIncrement();
          pq->add(tm,position);
        } else {

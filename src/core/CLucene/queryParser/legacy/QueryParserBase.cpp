...
//Post - A query instance has been returned for the specified field
  ...
  try{
    while (source->next(&t)){
      v.push_back(STRDUP_TtoT(t.termBuffer<TCHAR>()));
    ...
    if (v.size() == 1){
      ...
      if (severalTokensAtSamePosition) {
        if (positionCount == 1) {
          ...
          CLuceneStringArray::iterator itr = v.begin();
          while ( itr != v.end() ){
...
Query* QueryParserBase::ParseRangeQuery(const TCHAR* field, TCHAR* queryText, bool inclusive)
{
  ...
  do
  {
    ...
    if (tret)
    {
      if ( !from && _tcscmp(t.termBuffer<TCHAR>(),_T("TO"))==0 )
        continue;


      TCHAR* tmp = STRDUP_TtoT(t.termBuffer<TCHAR>());

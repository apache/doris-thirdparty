	...
	lazy(false)
{
	...
	_name        = Name;//CLStringIntern::intern( Name );
	fieldsData = reader;
...
const void* Field::rawStringValue() {return fieldsData;}
size_t Field::getFieldDataLength() { return fieldsDataLength; }
...
//CL_NS(analysis)::STokenStream* Field::stokenStreamValue() { return (valueType & VALUE_STOKENSTREAM) ? static_cast<CL_NS(analysis)::STokenStream*>(fieldsData) : NULL; }
...
bool 	Field::isTokenizedCHS() const	{
    return (config & INDEX_TOKENIZED) != 0 && (config & INDEX_CHS) != 0; }
...
void Field::setValue(TCHAR* value, const bool duplicateValue) {
	...
	if (duplicateValue)
		...
		fieldsData = value;
    fieldsDataLength = _tcslen(value);
	valueType = VALUE_STRING;
}

void Field::setValue(char* value, size_t fieldDataLen) {
    _resetValue();
    fieldsDataLength = fieldDataLen;
    fieldsData = value;
...
/*void Field::setValue(CL_NS(analysis)::STokenStream* value) {
    _resetValue();
    fieldsData = value;
    valueType = VALUE_STOKENSTREAM;
}*/

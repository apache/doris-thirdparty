...
public:
  ...
  * @add_local when calling stringToTime+timeToString together, set it to false avoid double add
  * @return the number of milliseconds since January 1, 1970, 00:00:00 GMT
  * @throws ParseException if <code>dateString</code> is not in the
  *  expected format
  */
  static int64_t stringToTime(const TCHAR* dateString, bool add_local=true, bool fill_date=false);
  ...
  * Only be called when time consisting date (no time) only:
  ...
  */
  ...
  inline static int64_t SecondsDifferenceFromGMT = 0;

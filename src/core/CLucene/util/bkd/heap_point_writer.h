...
namespace bkd
{
    ...
    public:
        ...
        // std::shared_ptr<std::vector<int64_t>> ords_long_;
        // std::shared_ptr<std::vector<int32_t>> ords_;
        ...
        const int32_t values_per_block_; // std::max(1, 4096/packedBytesLength)

        const bool single_value_per_doc_;
        // TODO: use shared_ptr to avoid vector copy
        //using vectorptr = std::shared_ptr<std::vector<uint8_t>>;
        //std::vector<vectorptr> blocks_; // vector<vectorptr<uint8>>

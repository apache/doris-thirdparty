...
namespace bkd {
    ...
    bkd_writer::bkd_writer(int32_t maxDoc, int32_t numDataDims,
        ...
        max_doc_ = maxDoc;
        max_depth_ = maxDepth;
        //docs_seen_ = std::make_unique<FixedBitSet>(maxDoc);
    ...
    void bkd_writer::add(std::vector<uint8_t> &packedValue, int32_t docID) {
        ...
        if (point_count_ > total_point_count_)
            _CLTHROWA(CL_ERR_IllegalArgument, ("total_point_count_=" + std::to_string(total_point_count_) + " was passed when we were created, but we just hit " + std::to_string(point_count_) + " values").c_str());
        //docs_seen_->Set(docID);
        //docs_seen_.insert(docID);
    }
    ...
    int64_t bkd_writer::finish(store::IndexOutput *out, store::IndexOutput *index_out) {
        if (point_count_ == 0) {
            return 0;
            //_CLTHROWA(CL_ERR_IllegalArgument, "must index at least one point");
        }
        ...
        //assert(point_count_ / num_leaves <= max_points_in_leaf_node_);
        ...
        build(1, num_leaves, sorted_point_writers,
              ...
              leaf_block_fps,
              to_close_heroically);
        //assert(temp_dir_->GetCreatedFiles().size() == 0);
        success = true;
        if (!success) {
            //TODO IOUtilas need to be implemented
        }
    ...
    int32_t bkd_writer::append_block(const std::shared_ptr<store::RAMOutputStream> &writeBuffer, ByteArrayList &blocks) {
        //TODO:efficiency need to check here
    ...
    int32_t bkd_writer::recurse_pack_index(const std::shared_ptr<store::RAMOutputStream> &writeBuffer, std::vector<int64_t> &leafBlockFPs,
        ...
        if (nodeID >= (int32_t) leafBlockFPs.size()) {
            int32_t leaf_id = nodeID - leafBlockFPs.size();
            // in the unbalanced case it's possible the left most node noly has one child
            ...
            //find common prefix with last split value in this dimension
            ...
            if (prefix < bytes_per_dim_) {
                ...
                //message += std::to_string(first_diff_byte_delta);
            ...
            //message += "]";
            //pack the prefix, split_dim and delta fist diff byte into a single vInt:
            int32_t code = (first_diff_byte_delta * (1 + bytes_per_dim_) + prefix) * num_index_dims_ + split_dim;
            //printf("write first_diff_byte_delta %d\n",code);
            writeBuffer->writeVInt(code);
            //write the split value, prefix coded as. out parent's split value:
            ...
            //blocks.push_back(std::vector<uint8_t>());
            ...
            writeBuffer->reset();
            // replace out placeholder
            // std::fill(blocks[idx_save]->begin(), blocks[idx_save]->begin() + num_bytes2, bytes2.begin());
            // bytes2.insert(bytes2.begin(), blocks[idx_save]->begin(), blocks[idx_save]->begin() + num_bytes2);
            //TODO:copy is inefficient,need to improve performance here.
            ...
            // restore last split values to what caller originally passed us:
    ...
    void bkd_writer::write_leaf_block_packed_values(store::IndexOutput *out,
        ...
        //int32_t prefix_len_sum = std::accumulate(common_prefix_lengths.begin(), common_prefix_lengths.end(), 0);
    ...
    const uint8_t *bkd_writer::mark_right_tree(int64_t rightCount, int32_t splitDim, std::shared_ptr<path_slice> &source, const std::shared_ptr<LongBitSet> &ordBitSet) const {
        ...
        //assert(reader->Next());
        if (reader != nullptr) {
            /*std::copy(reader->packed_value().begin() + splitDim * bytes_per_dim_,
                      reader->packed_value().begin() + splitDim * bytes_per_dim_ + bytes_per_dim_,
                      scratch1_.begin());*/
    ...
    void bkd_writer::build(int32_t nodeID, int32_t leafNodeOffset,
        ...
        if (nodeID >= leafNodeOffset) {
            ...
            for (int32_t dim = 0; dim < num_index_dims_; dim++) {
                ...
                sorted_dim = dim;
                /*int32_t prefix = common_prefix_lengths_[dim];
                if (prefix < bytes_per_dim_) {
                    int32_t cardinality = 1;
                    uint8_t previous = scratch1_[offset + prefix];
                    for (int64_t i = 1; i < source->count_; ++i) {
                        heap_source->read_packed_value((source->start_ + i), scratch2_);
                        uint8_t b = scratch2_[offset + prefix];
                        if (b != previous) {
                            cardinality++;
                            previous = b;
                        }
            ...
            if (prefix_len_sum == packed_bytes_length_) {
                ...
                // NOTE: just write runlen compressed docID, instead of bitmap chunks
                // because we found that much faster.
            ...
            int32_t split_dim;
            if (num_index_dims_ > 1) {
                //TODO:recomment here when we support multiple dimension
    ...
    std::shared_ptr<point_writer> bkd_writer::sort(int32_t dim) {
        ...
        if (heap_point_writer_ != nullptr) {
            // TODO: try to use move to save some atomic reference increment and decrement.

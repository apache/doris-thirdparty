...
int bkd_reader::read_meta(store::IndexInput* meta_in) {
    // Meta is written in the tail of index file.
    //in_->seek(in_->length() - sizeof(int32_t) - sizeof(int64_t));
    type = meta_in->readInt();
    indexFP = meta_in->readLong();
    // NOTE:if indexFP is 0, it means index file is empty.
    if (0 == indexFP) {
        return 0;
    } else {
        return 1;
...
void bkd_reader::read_index(store::IndexInput* index_in) {
    ...
    if (version_ >= bkd_writer::VERSION_PACKED_INDEX) {
        ...
        /*printf("read packed index start\n");
        for(auto i = packed_index_->begin();i != packed_index_->end();i++) {
            printf("%0x ",*i);
        }
...
int32_t bkd_reader::read_doc_ids(store::IndexInput *in, int64_t blockFP, bkd_docID_set_iterator *iter) const {
    ...
    int32_t count = in->readVInt();
    //docIds_writer::read_bitmap_ints(in, count, docIDs);
...
void bkd_reader::visit_unique_raw_doc_values(std::vector<uint8_t> &scratchPackedValue,
    ...
    //iter->docIDs_bitmap = std::move(iter->docIDs_bitmap_vector[0]);
...
int64_t bkd_reader::ram_bytes_used() {
    if (!packed_index_->empty()) {
        return packed_index_->capacity();// std::shared_ptr<std::vector<uint8_t>> packed_index_; 元素1字节
    } else {
        //  std::vector<uint8_t> split_packed_values_ // 元素1字节

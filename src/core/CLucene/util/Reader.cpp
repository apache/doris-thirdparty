...
/*template <typename T>
void SStringReader<T>::init(const T *_value, int32_t _length, bool copyData) {
...
int32_t StringReader::read(const void** start, int32_t min, int32_t max){
	if ( m_size == pos )
		return -1;
    *(const TCHAR**)start = this->value + pos;
...
int32_t AStringReader::read(const void** start, int32_t min, int32_t max){
    if ( m_size == pos )
        return -1;
...
int32_t FileInputStream::read(const void** start, int32_t min, int32_t max){
	return _internal->jsbuffer->read((const signed char*&)*start,min,max);
}
...
public:
	...
	protected:
		int readChar(){
			...
			if ( encoding == ASCII ){
				int32_t ret = this->input->read((const void**)&buf, 1, 1) ;
				if ( ret == 1 ){
					return buf[0];
				}else
					return -1;
				...
				int32_t ret = this->input->read((const void**)&buf, 2, 2);
				if ( ret < 0 )
					return -1;
				else if ( ret == 1 ){
					return buf[0];
				...
				if ( ret == 1 ){
					...
					if ( len > 1 ){
						*utf8buf = buf[0];
						ret = this->input->read((const void**)&buf, len-1, len-1);
					}else
						return buf[0];
...
int32_t SimpleInputStreamReader::read(const void** start, int32_t min, int32_t max){
	return _internal->jsbuffer->read((const TCHAR*&) *start, min, max);
}
...
public:
	...
	protected:
		int32_t fillBuffer(TCHAR* start, int32_t space){
			const TCHAR* buffer;
			int32_t r = input->read((const void**)&buffer, 1, space);
			if ( r > 0 )
...
int32_t FilteredBufferedReader::read(const void** start, int32_t min, int32_t max){
	return _internal->jsbuffer->read((const TCHAR*&)*start,min,max);
}
...
int32_t FilteredBufferedInputStream::read(const void ** start, int32_t min, int32_t max){
	return _internal->jsbuffer->read((const signed char *&) *start,min,max);
}

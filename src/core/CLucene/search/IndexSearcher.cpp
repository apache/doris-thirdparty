...
CL_NS_DEF(search)
  ...
  IndexSearcher::IndexSearcher(CL_NS(store)::Directory* directory, bool closeDirectory){
      //Func - Constructor
      ...
      reader = IndexReader::open(directory, closeDirectory);
      readerOwner = true;
  }
  ...
  void IndexSearcher::_search(Query* query, const Functor& cb) {
      CND_PRECONDITION(reader != NULL, "reader is NULL");
      CND_PRECONDITION(query != NULL, "query is NULL");

...
CL_NS_DEF(search)
    ...
    BooleanQuery::BooleanQuery(int32_t minimumNumberShouldMatch, const std::vector<BooleanClause*>& clauses):
        clauses(_CLNEW ClausesType(true))
    {
        minNrShouldMatch = minimumNumberShouldMatch;
        ...
        // todo: ComputeHashCode() and RewriteNoScoring() need this sets!
//        clause_sets_[BooleanClause::Occur::SHOULD] = std::make_shared<std::vector<QueryPtr>>();
//        clause_sets_[BooleanClause::Occur::MUST] = std::make_shared<std::vector<QueryPtr>>();
//        clause_sets_[BooleanClause::Occur::FILTER] = std::make_shared<std::vector<QueryPtr>>();
//        clause_sets_[BooleanClause::Occur::MUST_NOT] = std::make_shared<std::vector<QueryPtr>>();
    }
  ...
  void BooleanQuery::add(BooleanClause* clause) {
    if (clauses->size() >= getMaxClauseCount())
      _CLTHROWA(CL_ERR_TooManyClauses,"Too Many Clauses, please change your wildcard condition and try again");

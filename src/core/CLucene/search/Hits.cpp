...
const size_t DEFAULT_ONE_TIME_MORE_DOCS = 1000000;
const size_t DEFAULT_MAX_DOCS_CACHE = DEFAULT_ONE_TIME_MORE_DOCS*6;
	...
	Hits::Hits(Searcher* s, Query* q, Filter* f, const Sort* _sort, size_t nlimit):
		query(q), searcher(s), filter(f), sort(_sort) , _length(0), first(NULL), last(NULL),
		numDocs(0), maxDocs(DEFAULT_MAX_DOCS_CACHE), nDeletedHits(0), debugCheckedForDeletions(false),
		nLimit(nlimit)
	{
	//Func - Constructor
	...
	//Post - The instance has been created
		...
		size_t initial_read_docs = DEFAULT_ONE_TIME_MORE_DOCS;
		if (nLimit)
        {
            auto maxSize = std::min(nLimit, DEFAULT_ONE_TIME_MORE_DOCS);
            maxDocs = maxSize * 2;
            initial_read_docs = maxSize;
        }
		//retrieve "n" initially
		getMoreDocs(initial_read_docs);
	...
	void Hits::getMoreDocs(const size_t m){
		auto get_min = [needs = m](const size_t hits)
        {
		    size_t _min = needs;
		    if(hits > _min)
		        _min = hits;
		...
		size_t n = nLimit ? nLimit : get_min(hitDocs->size());

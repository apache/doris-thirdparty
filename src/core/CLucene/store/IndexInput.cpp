...
CL_NS_USE(util)
  ...
  void IndexInput::readBytes( uint8_t* b, const int32_t len, int32_t offset, bool /*useBuffer*/) {
      // Default to ignoring useBuffer entirely
...
BufferedIndexInput::BufferedIndexInput(int32_t _bufferSize):
    ...
    bufferPosition(other.bufferPosition)
  {
    ...
    **   if ( clone.buffer != NULL) { */
    ...
    void BufferedIndexInput::readBytes(uint8_t* b, const int32_t len, int32_t offset){
        readBytes(b, len, offset, true);
    }
    void BufferedIndexInput::readBytes(uint8_t* _b, const int32_t _len, bool useBuffer) {
        readBytes(_b, _len, 0, useBuffer);
    }
  void BufferedIndexInput::readBytes(uint8_t* _b, const int32_t _len, int32_t offset, bool useBuffer){
    int32_t len = _len;
    uint8_t* b = _b;
    ...
    if(len <= (bufferLength-bufferPosition)){
      ...
      if(len>0) // to allow b to be null if len is 0...
        memcpy(b + offset, buffer + bufferPosition, len);
      bufferPosition+=len;
    } else {
      // the buffer does not have enough data. First serve all we've got.
      int32_t available = bufferLength - bufferPosition;
      if(available > 0){
        memcpy(b + offset, buffer + bufferPosition, available);
        b += available;
        len -= available;
      ...
      if (useBuffer && len<bufferSize){
        ...
        if(bufferLength<len){
          ...
          memcpy(b + offset, buffer, bufferLength);

...
RAMOutputStream::RAMOutputStream() : file(_CLNEW RAMFile),
                                     deleteFile(true),
                                     currentBuffer(NULL),
                                     currentBufferIndex(-1),
                                     bufferPosition(0),
...
void RAMOutputStream::writeTo(std::vector<uint8_t> &bytes,
                              int offset) {
    flush();
    ...
    while (pos < end) {
        ...
        memcpy(bytes.data()+ bytesUpto, file->getBuffer(buffer++), length);
        bytesUpto += length;
        pos = nextPos;
    }
}
...
void RAMOutputStream::writeBytes(const uint8_t *b,  int32_t len,  int32_t offset) {
    int32_t srcOffset = offset;
    ...
    while (len > 0) {
        ...
        memcpy( currentBuffer+bufferPosition, b+offset, bytesToCopy * sizeof(uint8_t) );
        offset += bytesToCopy;
        len -= bytesToCopy;
...
RAMInputStream::RAMInputStream(const RAMInputStream &other) : IndexInput(other) {
    file = other.file;
...
void RAMInputStream::readBytes(uint8_t *_dest, const int32_t _len) {
    readBytes(_dest, _len, 0);
}
void RAMInputStream::readBytes(uint8_t *_dest, const int32_t _len, int32_t offset) {
    ...
    while (len > 0) {
        ...
        memcpy(dest + offset, currentBuffer + bufferPosition, bytesToCopy * sizeof(uint8_t));
...
RAMDirectory::RAMDirectory() : Directory(), files(_CLNEW FileMap(true, true)) {
    this->sizeInBytes = 0;
...
RAMDirectory::RAMDirectory(Directory *dir) : Directory(), files(_CLNEW FileMap(true, true)) {
    this->sizeInBytes = 0;
    setLockFactory(_CLNEW SingleInstanceLockFactory());
    _copyFromDir(dir, false);
}

RAMDirectory::RAMDirectory(const char *dir) : Directory(), files(_CLNEW FileMap(true, true)) {
    this->sizeInBytes = 0;
    setLockFactory(_CLNEW SingleInstanceLockFactory());
